...
def entropy(clasificaciones):
    counter = Counter(clasificaciones)
    entropia = 0.0
    cantidad_de_elementos = len(clasificaciones)
    for clasificacion in clasificaciones:
        probabilidad = counter[clasificacion] / float(cantidad_de_elementos)
        entropia += -probabilidad * log2(probabilidad)

    return entropia


def information_gain(valores_de_atributo, clasificaciones):
    contadores_por_valor = contar_valores_de_atributo(clasificaciones, valores_de_atributo)
    reduccion_de_entropia = calcular_reduccion_de_entropia(clasificaciones, contadores_por_valor)
    return entropy(clasificaciones) - reduccion_de_entropia


def calcular_reduccion_de_entropia(clasificaciones, contadores_por_valor):
    reduccion_de_entropia = 0.0
    for contador in contadores_por_valor:
        clasificaciones_por_valor = contadores_por_valor[contador]

        clasificaciones_de_atributo = []
        for clasificacion in clasificaciones_por_valor:
            conteo_para_la_clasificacion = clasificaciones_por_valor[clasificacion]
            for _ in range(conteo_para_la_clasificacion):
                clasificaciones_de_atributo.append(clasificacion)

        reduccion_de_entropia += len(clasificaciones_de_atributo) * entropy(clasificaciones_de_atributo)
    return reduccion_de_entropia / len(clasificaciones)


def contar_valores_de_atributo(clasificaciones, valores_de_atributo):
    contadores_por_valor = {valor: {clasificacion: 0 for clasificacion in clasificaciones} for valor in
                            valores_de_atributo}
    for index, valor in enumerate(valores_de_atributo):
        clasificacion = clasificaciones[index]
        contadores_por_valor[valor][clasificacion] += 1
    return contadores_por_valor
...
class DecisionTree():
    ...
    def predict(self, Xtest):
        ...
        for irow in range(Xtest.shape[0]):
            ...
            # mitigar el problema
            try:
                res.append(self._predict(row))
            except:
                res.append(Counter(self._fit_y).most_common(1)[0][0])
    ...
    def plot_graph(self):
        ...
        def _tree_layout(xcenter, ycenter, dt):
            ...
            for k, sdt in dt.sub_trees.iteritems():
                res += _tree_layout((xcenter + np.linspace(-.5 / float(abs(ycenter - 1) ** 2),
                                                           0.5 / float(abs(ycenter - 1) ** 2), len(dt.sub_trees)))[i],
                                    ycenter - 1, sdt)
                i += 1
            return res
        ...
        def _create_dict_id_name(dt):
            res = []
            if dt.attr == "":
                res.append((id(dt), dt.predicted_class))
            else:
                res.append((id(dt), dt.attr))
        ...
        for k, coord in pos.iteritems():
            ...
            pylab.text(coord[0], coord[1], dict_node_id_to_label[k], horizontalalignment='center',
        ...
        for src, dst, label in edges:
            ...
            pylab.text(src_point[0] + (dst_point[0] - src_point[0]) / 2.,
                       src_point[1] + (dst_point[1] - src_point[1]) / 2., label,
